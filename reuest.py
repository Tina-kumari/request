...
serial_number=1
for i in api_data:
    print(serial_number,".",i["name"],":","id",i["id"])
    serial_number+=1

course_no=int(input("choose your course number:"))
print(api_data[course_no-1]["name"])
id=api_data[course_no-1]["id"]
N_url=requests.get("http://api.merakilearn.org/courses/"+str(id)+"/exercises")
data=N_url.json()
...
if nevigation=="p":
    ...
    for i in api_data:
        ...
        serial_number+=1
    course_no=int(input("enter your number do u want:"))
    print(api_data[course_no-1]["name"])
